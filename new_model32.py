...
if True: # T/T vs rho J a at diff D
    
    Dvals = np.array([0]) #-1/1000,0,1/1000,1/100,1/10]);
    for Di in range(len(Dvals)):
        ...
        Rvals = np.empty((len(Evals),3), dtype = float);
        Tvals = np.empty((len(Evals),3), dtype = float);
        for Evali in range(len(Evals)):
            ...
            # wfm.kernel returns tuple (R coefs, T coefs)
        ...
        fname = "data/new_model32/D"+str(int(D*1000)/1000);
...
if False: 
    ...
    for J12i in range(len(J12vals)):
        ...
        data[2:2+len(source),:] = Tvals.T;
        data[2+len(source):2+2*len(source),:] = Rvals.T;
        fname = "data/new_model32/J12"+str(int(DeltaD/J12));
        print("Saving data to "+fname);
        np.save(fname, data);
